...
function rbfga(shapeparam,xk,xe,A)
	mat"addpath('src/external/rbfga')"
	N = size(A,1)
	for i = 1:N
		 println("i = ", i)
		mat"$w = rbfga_weights('0',$shapeparam,$xk,$xe($i,:));"
		A[i,:] .= vec(w)
	end
end
...
#xk = copy(Bgridpert')#xkuni
#nxk = nxkuni


xk = 2*(halton(round(Int64,26/pi*8),2) .- 0.5)
nxk = size(xk,1)
#xk = xkunion
#nxk = 66
...
#Bgriduni = hcat(Bgriduni,Bgrid)
#norderuni2 = norderuni^2 + norderrbf^2

#Aga = Matrix{Float64}(undef,norderrbf^2,norderuni2);
#rbfga(1e-3,copy(Bgriduni'),copy(Bgrid'),Aga)
topline = 0.25
bottomline = -0.25
...
#F2d(x,y) = (y > topline) * Tm(x / (boxlen/2)) + (y<bottomline)	*Tn(y / (boxlen/2))
Fxk = F2d.(xk[:,1],xk[:,2])
...
for n = 1:norder^2
	...
	# if (B0grid[2,n] > topline) || (B0grid[2,n] < bottomline)
		push!(idxB0in,n)
	else
		push!(idxB0out,n)
	end
...
#	 	 if (Bgrid[2,n] > topline) || (Bgrid[2,n] < bottomline)
		push!(idxBin,n)
	else
		push!(idxBout,n)
	end
...
#	 	 if (xk[n,2] > topline) || (xk[n,2] < bottomline)
		push!(idxxkin,n)
	else
		push!(idxxkout,n)
	end
...
#	 	 if (compBgrid[2,n] > topline) || (compBgrid[2,n] < bottomline)
		push!(idxcompBin,n)
	else
		push!(idxcompBout,n)
	end
...
#if (Bgriduni[2,n] > topline) || (Bgriduni[2,n] < bottomline)
...
#		  println("Max err = ", ERROR)
		    push!(idxxrandin,i)
	 	 err = abs(F2d(xrandeval[i,1],xrandeval[i,2]) - eval_func_2D(chebcoefs,xrandeval[i,1],xrandeval[i,2],norderrbf,-boxlen/2,boxlen/2,norderrbf,-boxlen/2,boxlen/2))
		  	push!(errlocal,err)
#		  println("error = ", err)
	   if err > maxerror
			 maxerror = err
