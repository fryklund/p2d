...
%RBFGA_WEIGHTS  Compute differentiation weights using the RBF-GA algorithm.
%   weights = rbfga_weights(op,ep,xi,xc,opts)
% 
%     op - operator(s) e.g., '0','x','xy' or 'L', or {'x','xy','L1'}
%     ep - shape parameter (where 0 corresponds to the flat limit)
...
%
%   Example usage: (Compute Laplacian weights on a 3x3 lattice)
%     x = linspace(-1,1,3);
%     [X,Y] = meshgrid(x);
%     xi = [X(:) Y(:)];
%     w = rbfga_weights('L',0.01,xi,[0 0]);
%
%   For more information, see
%     Fornberg, Bengt, Erik Lehto, and Collin Powell. 
%     "Stable calculation of Gaussian-based RBF-FD stencils."
%     Computers & Mathematics with Applications 65.4 (2013): 627-637.
%
%   Copyright (c) 2012, Erik Lehto
        ...
        switch op{i}
            ...
            case 'xy'
                rhs(rows,i) = (1/h^2)*4*exp(2*k)/ep^(2*(k-2))*...
                    (B*(xi(1:nb,1).*xi(1:nb,2).*Gk(0,k-2))); %d2/dxdy
            case 'xz'
                rhs(rows,i) = (1/h^2)*4*exp(2*k)/ep^(2*(k-2))*...
                    (B*(xi(1:nb,1).*xi(1:nb,3).*Gk(0,k-2))); %d2/dxdz
            case 'yy'
                rhs(rows,i) = (1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
            ...
            case 'yz'
                rhs(rows,i) = (1/h^2)*4*exp(2*k)/ep^(2*(k-2))*...
                    (B*(xi(1:nb,2).*xi(1:nb,3).*Gk(0,k-2))); %d2/dydz
            case 'zz'
                rhs(rows,i) = (1/h^2)*2*exp(2*k)/ep^(2*(k-1))*...
                    (B*(-Gk(0,k)*ones(nb,1) + ...
...
P = zeros(n,nchoosek(p+3,3));  % make room for 3D Vandermonde
i = 1;
for k = 0:p
    for l = k:-1:0
        for m=k-l:-1:0
